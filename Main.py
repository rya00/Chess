...
# Load each individual chess piece image of all images to be displayed
...
def draw_board(screen):
    ...
    for r in range(DIMENSION):
        for c in range(DIMENSION):
            # find the color based on even or odd position
            color = colors[((r + c)%2)]                                                             
        ...
        if game_state.board[row][col][0] == ('w' if game_state.white_to_move else 'b'):             
            ...
            for move in valid_moves:
                if move.start_row == row and move.start_col == col:
                    ...
                    s.fill(pg.Color(hightlight_color))                                              
...
def draw_pieces(screen, board):
    ...
    for r in range(DIMENSION):
        for c in range(DIMENSION):
            ...
            piece = board[r][c]                                                                     
            # if empty piece go to the next piece
            if piece != "--":                                                                       
